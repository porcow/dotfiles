...
(setq display-time-load-average nil
      display-time-format "%l:%M %p %a %d %b"
...
(defvar-local center-document-desired-width-function nil
  "Optional function of WINDOW returning desired content width in columns.
Use this to compute per-buffer widths (e.g., account for text scaling or font).")

(defun center-document--desired-width (window)
  (let ((fn (buffer-local-value 'center-document-desired-width-function (window-buffer window))))
    (cond
     ((functionp fn) (max 0 (truncate (funcall fn window))))
     (t (buffer-local-value 'center-document-desired-width (window-buffer window))))))

(defun center-document--margin-width (window)
  (let* ((available (window-total-width window))
         (desired (center-document--desired-width window)))
    (max 0 (truncate (/ (- available desired) 2.0)))))

(defun center-document--adjust-window (window)
...
(defun center-document--adjust-windows (&optional frame)
  (dolist (window (window-list frame 'no-mini))
    (center-document--adjust-window window)))

(defun center-document--buffers-with-mode ()
  (catch 'found
    (dolist (buffer (buffer-list))
      (when (buffer-local-value 'center-document-mode buffer)
        (throw 'found t)))
    nil))

(defun center-document--install-hooks ()
  (unless center-document--hooks-installed
    (add-hook 'window-size-change-functions #'center-document--adjust-windows)
    (add-hook 'window-configuration-change-hook #'center-document--adjust-windows)
    (setq center-document--hooks-installed t)))

(defun center-document--maybe-remove-hooks ()
  (unless (center-document--buffers-with-mode)
    (remove-hook 'window-size-change-functions #'center-document--adjust-windows)
    (remove-hook 'window-configuration-change-hook #'center-document--adjust-windows)
    (setq center-document--hooks-installed nil)))
